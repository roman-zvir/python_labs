...
def refresh_tree(filter_text=''):
    ...
    for name, experience, department in db_get():
        if (filter_text.lower() in name.lower() or
            filter_text.lower() in experience.lower() or
            filter_text.lower() in department.lower()):
...
tk.Button(frm_search, text='Go', command=lambda: refresh_tree(search_var.get())).pack(side='left', padx=2)
tk.Button(frm_search, text='Clear', command=lambda: (search_var.set(''), refresh_tree(''))).pack(side='left')
...
tree.heading('Experience', text='Experience', command=lambda: sort_column(tree, 'Experience'))
tree.heading('Department', text='Department', command=lambda: sort_column(tree, 'Department'))
