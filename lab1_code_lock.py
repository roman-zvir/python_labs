...
for text, row, col in buttons:
    button = tk.Button(
        frame_keyboard, width=5, text=text, command=lambda t=text: handle_digit(t)
    )
    button.grid(row=row, column=col, padx=2, pady=2)

# Control buttons
button_back = tk.Button(
    frame_keyboard, width=5, text="Clear", bg="blue", command=handle_back
)
button_back.grid(row=3, column=0, padx=2, pady=2)

button_enter = tk.Button(
    frame_keyboard, width=5, text="Enter", bg="green", command=handle_enter
)
